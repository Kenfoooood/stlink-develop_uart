...
int stlink_reset(stlink_t *sl, enum reset_type type) {
  ...
  if (type == RESET_AUTO) {
    ...
    emulation_read();//stlink_read_debug32(sl, STLINK_REG_DHCSR, &dhcsr);
  }
...
// 322
int stlink_target_connect(stlink_t *sl, enum connect_type connect, uint32_t *mem) {
  ...
  return 0;
  //return stlink_load_device_params(sl); hui
}
