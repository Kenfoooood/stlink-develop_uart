...
int main(int argc, char** argv) {
    ...
    //sl = stlink_open_usb(state.logging_level, state.connect_mode, state.serialnumber, state.freq);hui
    if (sl == NULL) { return(1); }
    ...
    do {                            // don't go beserk if serve() returns with error
        if (serve(sl, &state)) { usleep (1 * 1000); } //1

        sl = connected_stlink;      // in case serve() changed the connection
        stlink_run(sl, RUN_NORMAL); // continue  //common.c
...
winsock_error:
    ...
    // switch back to mass storage mode before closing
    //stlink_exit_debug_mode(sl);потом восстановить
    //stlink_close(sl);
...
int serve(stlink_t *sl, st_state_t *st) {
    ...
    SOCKET client = accept(sock, (struct sockaddr*)&clientInfo, &clientInfo_size);
    ...
    while (1) {
        ...
        case 'k':
            ...
            //sl = stlink_open_usb(st->logging_level, st->connect_mode, st->serialnumber, st->freq);hui
