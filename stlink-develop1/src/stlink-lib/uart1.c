...
/*void _stlink_uart_close(stlink_t* sl) {
    if (!sl) { return; }

    struct stlink_libusb * const handle = sl->backend_data;

    // maybe we couldn't even get the uart device?
    if (handle != NULL) {
        if (handle->uart_handle != NULL) { libusb_close(handle->uart_handle); }
...
/*ssize_t send_recv(struct stlink_libusb* handle, int terminate,
    ...
    while (1) {
        res = 0;
        t = libusb_bulk_transfer(handle->uart_handle, handle->ep_req, txbuf, (int)txsize, &res, 3000);
        ...
        if (rxsize != 0) {
            t = libusb_bulk_transfer(handle->uart_handle, handle->ep_rep, rxbuf, (int)rxsize, &res, 3000);
        ...
        if ((handle->protocoll == 1) && terminate) {
            ...
            t = libusb_bulk_transfer(handle->uart_handle, handle->ep_rep, sg_buf, 13, &res, 3000);
...
/*int _stlink_uart_version(stlink_t *sl) {
    struct stlink_libusb * const slu = sl->backend_data;
...
/*int32_t _stlink_uart_target_voltage(stlink_t *sl) {
    struct stlink_libusb * const slu = sl->backend_data;
...
/*int _stlink_uart_read_debug32(stlink_t *sl, uint32_t addr, uint32_t *data) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const rdata = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;
    ssize_t size;
...
#define MEM 288*1024
uint32_t flash_memory[MEM];


int emulation_read() {
	DLOG(" *** Emulation read *** \n");
	flash_memory[0] = 228;
	printf("address: %#010x - value: %d\n", flash_memory, *flash_memory);
	flash_memory[1] = 1;
	printf("address: %#010x - value: %d\n", flash_memory+1, *(flash_memory+1));
	printf("address: %#010x - value: %d\n", flash_memory+2, *(flash_memory+2));
	return 0;
}


int _stlink_uart_write_debug32(stlink_t *sl, uint32_t addr, uint32_t data) {
    /*struct stlink_libusb* const slu = sl->backend_data;
    unsigned char* const rdata = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;*/
    ssize_t size;
...
/*int _stlink_uart_get_rw_status(stlink_t *sl) {
...
/*int _stlink_uart_write_mem32(stlink_t *sl, uint32_t addr, uint16_t len) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;
    int i, ret;
    ...
    return(_stlink_uart_get_rw_status(sl));
}*/

/*int _stlink_uart_write_mem8(stlink_t *sl, uint32_t addr, uint16_t len) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;
    int i, ret;
...
/*int _stlink_uart_current_mode(stlink_t * sl) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const cmd  = sl->c_buf;
    unsigned char* const data = sl->q_buf;
    ssize_t size;
...
/*int _stlink_uart_core_id(stlink_t * sl) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const cmd  = sl->c_buf;
    unsigned char* const data = sl->q_buf;
    ssize_t size;
...
/*int _stlink_uart_status_v2(stlink_t *sl) {
    int result;
    uint32_t status = 0;

    result = _stlink_uart_read_debug32(sl, STLINK_REG_DHCSR, &status);
...
/*int _stlink_uart_status(stlink_t * sl) {
    if (sl->version.jtag_api != STLINK_JTAG_API_V1) { return(_stlink_uart_status_v2(sl)); }
...
/*int _stlink_uart_force_debug(stlink_t *sl) {
    struct stlink_libusb *slu = sl->backend_data;

    int res;

    if (sl->version.jtag_api != STLINK_JTAG_API_V1) {
        res = _stlink_uart_write_debug32(sl, STLINK_REG_DHCSR, STLINK_REG_DHCSR_DBGKEY | STLINK_REG_DHCSR_C_HALT | STLINK_REG_DHCSR_C_DEBUGEN);
        return(res);
    }
...
/*int _stlink_uart_enter_swd_mode(stlink_t * sl) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const cmd  = sl->c_buf;
    ssize_t size;
    unsigned char* const data = sl->q_buf;
...
/*int _stlink_uart_exit_dfu_mode(stlink_t* sl) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const cmd = sl->c_buf;
    ssize_t size;
...
/*int _stlink_uart_reset(stlink_t * sl) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd = sl->c_buf;
    ssize_t size;
...
/*int _stlink_uart_jtag_reset(stlink_t * sl, int value) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd = sl->c_buf;
    ssize_t size;
...
/*int _stlink_uart_step(stlink_t* sl) {
    struct stlink_libusb * const slu = sl->backend_data;

    if (sl->version.jtag_api != STLINK_JTAG_API_V1) {
        // emulates the JTAG v1 API by using DHCSR
        _stlink_uart_write_debug32(sl, STLINK_REG_DHCSR, STLINK_REG_DHCSR_DBGKEY | STLINK_REG_DHCSR_C_HALT |
                                                        STLINK_REG_DHCSR_C_MASKINTS | STLINK_REG_DHCSR_C_DEBUGEN);
        _stlink_uart_write_debug32(sl, STLINK_REG_DHCSR, STLINK_REG_DHCSR_DBGKEY | STLINK_REG_DHCSR_C_STEP |
                                                        STLINK_REG_DHCSR_C_MASKINTS | STLINK_REG_DHCSR_C_DEBUGEN);
        return _stlink_uart_write_debug32(sl, STLINK_REG_DHCSR, STLINK_REG_DHCSR_DBGKEY | STLINK_REG_DHCSR_C_HALT |
 ...
 * @param type
 */
int _stlink_uart_run(stlink_t* sl, enum run_type type) {
    //struct stlink_libusb * const slu = sl->backend_data;

    int res;

    if (sl->version.jtag_api != STLINK_JTAG_API_V1) {
        res = _stlink_uart_write_debug32(sl, STLINK_REG_DHCSR, 
...
/*int _stlink_uart_set_swdclk(stlink_t* sl, int clk_freq) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd = sl->c_buf;
    ssize_t size;
...
/*int _stlink_uart_exit_debug_mode(stlink_t *sl) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const cmd = sl->c_buf;
    ssize_t size;
    int i = fill_command(sl, SG_DXFER_FROM_DEV, 0);
...
int _stlink_uart_read_mem32(stlink_t *sl, uint32_t addr, uint16_t len) {
    /*struct stlink_libusb* const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd = sl->c_buf;
    ssize_t size;
...
int _stlink_uart_read_all_regs(stlink_t *sl, struct stlink_reg *regp) {
    /*struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const cmd = sl->c_buf;
    unsigned char* const data = sl->q_buf;
    ssize_t size;
	...
	regp->xpsr       = 0x01000000;
    regp->main_sp    = 0x20000c80;
    regp->process_sp = 0x00000000;
    regp->rw         = 0x00000000;
    regp->rw2        = 0x00000000;
...
int _stlink_uart_read_reg(stlink_t *sl, int r_idx, struct stlink_reg *regp) {
    /*struct stlink_libusb* const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;
    ssize_t size;
    ...
    sl->q_len = (int)size;
    stlink_print_data(sl);
    uint32_t r = read_uint32(sl->q_buf, reg_offset);
...
int _stlink_uart_read_unsupported_reg(stlink_t *sl, int r_idx, struct stlink_reg *regp) {
    uint32_t r;
    int ret = 0;
    ...
    //ret = _stlink_uart_write_mem32(sl, STLINK_REG_DCRSR, 4);

    if (ret == -1) { return(ret); }

    ret = _stlink_uart_read_mem32(sl, STLINK_REG_DCRDR, 4);
...
/*int _stlink_uart_read_all_unsupported_regs(stlink_t *sl, struct stlink_reg *regp) {
    int ret;

    ret = _stlink_uart_read_unsupported_reg(sl, 0x14, regp);

    if (ret == -1) { return(ret); }

    ret = _stlink_uart_read_unsupported_reg(sl, 0x21, regp);

    if (ret == -1) { return(ret); }

    for (int i = 0; i < 32; i++) {
        ret = _stlink_uart_read_unsupported_reg(sl, 0x40 + i, regp);
...
/* See section C1.6 of the ARMv7-M Architecture Reference Manual */
/*int _stlink_uart_write_unsupported_reg(stlink_t *sl, uint32_t val, int r_idx, struct stlink_reg *regp) {
    int ret;
    ...
    ret = _stlink_uart_write_mem32(sl, STLINK_REG_DCRDR, 4);
    ...
    return(_stlink_uart_write_mem32(sl, STLINK_REG_DCRSR, 4));
}*/

int _stlink_uart_write_reg(stlink_t *sl, uint32_t reg, int idx) {
    /*struct stlink_libusb* const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;*/
    ssize_t size;
...
/*int _stlink_uart_enable_trace(stlink_t* sl, uint32_t frequency) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;
    ssize_t size;
...
/*int _stlink_uart_disable_trace(stlink_t* sl) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;
    ssize_t size;
...
/*int _stlink_uart_read_trace(stlink_t* sl, uint8_t* buf, size_t size) {
    struct stlink_libusb * const slu = sl->backend_data;
    unsigned char* const data = sl->q_buf;
    unsigned char* const cmd  = sl->c_buf;
    uint32_t rep_len = 2;
    ...
    if (trace_count != 0) {
        int res = 0;
        int t = libusb_bulk_transfer(slu->uart_handle, slu->ep_trace, buf, trace_count, &res, 3000);
...
/*static stlink_backend_t _stlink_uart_backend = {
    _stlink_uart_close,
    _stlink_uart_exit_debug_mode,
    _stlink_uart_enter_swd_mode,
    NULL, // don't enter_jtag_mode here...
    _stlink_uart_exit_dfu_mode,
    _stlink_uart_core_id,
    _stlink_uart_reset,
    _stlink_uart_jtag_reset,
    _stlink_uart_run,
    _stlink_uart_status,
    _stlink_uart_version,
    _stlink_uart_read_debug32,
    _stlink_uart_read_mem32,
    _stlink_uart_write_debug32,
    _stlink_uart_write_mem32,
    _stlink_uart_write_mem8,
    _stlink_uart_read_all_regs,
    _stlink_uart_read_reg,
    _stlink_uart_read_all_unsupported_regs,
    _stlink_uart_read_unsupported_reg,
    _stlink_uart_write_unsupported_reg,
    _stlink_uart_write_reg,
    _stlink_uart_step,
    NULL,//_stlink_uart_current_mode,
    _stlink_uart_force_debug,
    _stlink_uart_target_voltage,
    _stlink_uart_set_swdclk,
    _stlink_uart_enable_trace,
    _stlink_uart_disable_trace,
    _stlink_uart_read_trace
};*/

static stlink_backend_t _stlink_uart_backend_NULL = {
    NULL, //_stlink_uart_close,
    NULL,//_stlink_uart_exit_debug_mode,
    NULL,//_stlink_uart_enter_swd_mode,
    NULL, // don't enter_jtag_mode here...
    NULL,//_stlink_uart_exit_dfu_mode,
    NULL,//_stlink_uart_core_id,
    NULL,//_stlink_uart_reset,
    NULL,//_stlink_uart_jtag_reset,
    _stlink_uart_run,
    NULL,//_stlink_uart_status,
    NULL,//_stlink_uart_version,
    emulation_read,//_stlink_uart_read_debug32,
    _stlink_uart_read_mem32,
    _stlink_uart_write_debug32,
    NULL,//_stlink_uart_write_mem32,
    NULL,//_stlink_uart_write_mem8,
    _stlink_uart_read_all_regs,
    _stlink_uart_read_reg,
    NULL,//_stlink_uart_read_all_unsupported_regs,
    _stlink_uart_read_unsupported_reg,
    NULL,//_stlink_uart_write_unsupported_reg,
    NULL,//_stlink_uart_write_reg,
    NULL,//_stlink_uart_step,
    NULL,//_stlink_uart_current_mode,
    NULL,//_stlink_uart_force_debug,
    NULL,//_stlink_uart_target_voltage,
    NULL,//_stlink_uart_set_swdclk,
    NULL,//_stlink_uart_enable_trace,
    NULL,//_stlink_uart_disable_trace,
    NULL,//_stlink_uart_read_trace
};
...
/*stlink_t *stlink_open_uart(enum ugly_loglevel verbose, enum connect_type connect, char serial[STLINK_SERIAL_BUFFER_SIZE], int freq) {
    stlink_t* sl = NULL;
    ...
    ugly_init(verbose);
    sl->backend = &_stlink_uart_backend;
    sl->backend_data = slu;
    ...
    while (cnt-- > 0) {
        ...
        if (desc.idVendor != STLINK_uart_VID_ST) { continue; }
        ...
        if (((serial == NULL) || (*serial == 0)) || (memcmp(serial, &sl->serial, STLINK_SERIAL_LENGTH) == 0)) {
            if (STLINK_V1_uart_PID(desc.idProduct)) {
                slu->protocoll = 1;
                sl->version.stlink_v = 1;
            } else if (STLINK_V2_uart_PID(desc.idProduct) || STLINK_V2_1_uart_PID(desc.idProduct)) {
                sl->version.stlink_v = 2;
            } else if (STLINK_V3_uart_PID(desc.idProduct)) {
    ...
    if (cnt < 0) {
        ...
        ret = libusb_open(list[cnt], &slu->uart_handle);
...
// libusb_kernel_driver_active is not available on Windows.
#if !defined(_WIN32)
    if (libusb_kernel_driver_active(slu->uart_handle, 0) == 1) {
        ret = libusb_detach_kernel_driver(slu->uart_handle, 0);
    ...
    if (libusb_get_configuration(slu->uart_handle, &config)) {
    ...
    if (config != 1) {
        ...
        if (libusb_set_configuration(slu->uart_handle, 1)) {
            // this may fail for a previous configured device
    ...
    if (libusb_claim_interface(slu->uart_handle, 0)) {
        WLOG("Stlink uart device found, but unable to claim (probably already in use?)\n");
        goto on_libusb_error;
    }
    ...
    if (desc.idProduct == STLINK_uart_PID_STLINK_NUCLEO ||
        desc.idProduct == STLINK_uart_PID_STLINK_32L_AUDIO ||
        desc.idProduct == STLINK_uart_PID_STLINK_V2_1 ||
        desc.idProduct == STLINK_uart_PID_STLINK_V3_uartLOADER ||
        desc.idProduct == STLINK_uart_PID_STLINK_V3E_PID ||
        desc.idProduct == STLINK_uart_PID_STLINK_V3S_PID ||
        desc.idProduct == STLINK_uart_PID_STLINK_V3_2VCP_PID ||
        desc.idProduct == STLINK_uart_PID_STLINK_V3_NO_MSD_PID) {
    ...
    if (connect == CONNECT_UNDER_RESET) {
        ...
        if (mode == STLINK_DEV_DEBUG_MODE) {
            ...
            _stlink_uart_exit_debug_mode(sl);
        }

        _stlink_uart_jtag_reset(sl, STLINK_DEBUG_APIV2_DRIVE_NRST_LOW);
    }
    ...
    _stlink_uart_set_swdclk(sl, freq);
...
/*static size_t stlink_probe_uart_devs(libusb_device **devs, stlink_t **sldevs[], enum connect_type connect, int freq) {
    stlink_t **_sldevs;
    libusb_device *dev;
    int i = 0;
    size_t slcnt = 0;
 ...
 /*   while ((dev = devs[i++]) != NULL) {
        ...
        if (!STLINK_SUPPORTED_uart_PID(desc.idProduct)) {
 ...
 /*   i = 0;
    ...
    while ((dev = devs[i++]) != NULL) {
        ...
        if (!STLINK_SUPPORTED_uart_PID(desc.idProduct)) { continue; }
        ...
        if (ret < 0) {
            if (ret == libusb_ERROR_ACCESS) {
                ELOG("Could not open uart device %#06x:%#06x, access error.\n", desc.idVendor, desc.idProduct);
            } else {
                ELOG("Failed to open uart device %#06x:%#06x, libusb error: %d)\n", desc.idVendor, desc.idProduct, ret);
            }
        ...
        stlink_t *sl = stlink_open_uart(0, connect, serial, freq);

        if (!sl) {
            ELOG("Failed to open uart device %#06x:%#06x\n", desc.idVendor, desc.idProduct);
            continue;
        }
...
/*size_t stlink_probe_uart(stlink_t **stdevs[], enum connect_type connect, int freq) {
    libusb_device **devs;
    stlink_t **sldevs;
    ...
    slcnt = stlink_probe_uart_devs(devs, &sldevs, connect, freq);
...
/*void stlink_probe_uart_free(stlink_t ***stdevs, size_t size) {
...
void init_emulation_connection(stlink_t *sl) {
	sl->backend = &_stlink_uart_backend_NULL;
	sl->version.stlink_v = 0;
	sl->freq = 2000;
	sl->chip_id = 0x421; //f446
	sl->core_id = 0x4ba00477; //F4 JTAG
	//stlink_target_connect(sl, CONNECT_UNDER_RESET);
}